...
export default function Clients() {
  ...
  listAll(ref(getStorage(app), "/clients")).then(res => {
      const imgs = document.querySelectorAll("[data-bg-image='ava']");

      res.items.forEach((e, i) => {
        getDownloadURL(ref(getStorage(app), e.fullPath) ).then(url => imgs[i].setAttribute('src', url))
      })
    ...
    dataForCard = dataForCard.map((e, i) => {
      return (
        <div className="flex flex-col items-center justify-between p-8 border-2 border-blue-950 rounded-2xl" key={i}>
          <img className="w-32 h-32 rounded-full" data-bg-image="ava" alt="" />
